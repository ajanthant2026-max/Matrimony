...
export type SyncPayload = {
  entityType: "USER" | "LIKE" | "MATCH" | "MESSAGE";
  entityId: string;
 ...
 */
export async function enqueueSync(payload: SyncPayload): Promise<boolean> {
  if (!syncQueue) {
  ...
  try {
    ...
    console.error("❌ Failed to publish message to queue:", (error as Error).message);
    return false;
  }
}
