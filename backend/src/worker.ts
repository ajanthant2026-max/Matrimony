...
import { connection, SYNC_QUEUE_NAME, SyncPayload } from "./queue.js";
...
async function logSyncAudit(
  ...
  const logMsg = `[SyncAuditLog] ${entityType} | ID: ${entityId} | Action: ${action} | Status: ${status} | Attempt: ${attempts}${
  ...
  try {
    ...
    console.warn(
      `⚠️ Unable to write SyncAuditLog to PostgreSQL (Database is likely offline): ${(err as Error).message}`
    );
  }
}
 ...
 */
export async function processSyncJob(job: Job<SyncPayload>) {
...
export let databaseSyncWorker: Worker | null = null;

export function startSyncWorker() {
  if (databaseSyncWorker) return;
  ...
  databaseSyncWorker.on("failed", (job, err) => {
    if (job) {
      console.warn(
        `🚨 Sync Job failed: [Job ID: ${job.id}]. Attempts made: ${job.attemptsMade}/${job.opts.attempts}. Next retry in progress. Error: ${err.message}`
      );
    } else {
