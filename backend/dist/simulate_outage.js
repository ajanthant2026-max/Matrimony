...
async function mockWriteToPostgreSQL(jobName, data) {
...
async function runSimulation() {
    ...
    if (isRedisAvailable) {
        ...
        // 2. Setup the Worker
        const simWorker = new Worker(SIMULATION_QUEUE_NAME, async (job) => {
        ...
        simWorker.on("failed", (job, err) => {
            if (job) {
                console.warn(`⚠️ Job "${job.name}" FAILED: ${err.message}. Retrying soon...`);
            }
        });
